...
def make_dict(adpd, dic):
    ...
    for h in os.listdir(adpd):
        if h != 'logs' and not h.endswith('.txt'):
                            ...
                            if os.path.isdir(adpd + '/' + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + call_macs2 + '/execution/'):
                                for l in os.listdir(adpd + '/' + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + call_macs2 + '/execution/'):
                                    if l.endswith('fc.signal.bigwig'):
                                        ...
                                        if name not in dic:
                                            dic[name] = {'fc_bigwig': json_item, 'pval_bigwig': '', 'overlap_hammock': '', 'idr_hammock': ''}
                                        else:
                                    ...
                                    if l.endswith('pval.signal.bigwig'):
                                        ...
                                        json_item = '{\"type\": '+ftype+',\"url\": '+url+',\"mode\": '+mode+',\"name\": '+tname+',\"qtc\": '+qtc+'}'
                                        if name not in dic:
                                            dic[name] = {'fc_bigwig': '', 'pval_bigwig': json_item, 'overlap_hammock': '', 'idr_hammock': ''}
                                        else:
                                            dic[name]['pval_bigwig'] = json_item
                            else:
                                if name not in dic:
                                    dic[name] = {'fc_bigwig': '', 'pval_bigwig': '', 'overlap_hammock': '', 'idr_hammock': ''}
                        if os.path.isdir(adpd + '/' + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + '/call-reproducibility_overlap/execution/'):
                            for m in os.listdir(adpd + '/' + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + '/call-reproducibility_overlap/execution/'):
                                if m.endswith('optimal_peak.narrowPeak.hammock.gz'):
                                    ...
                                    if name not in dic:
                                        dic[name] = {'fc_bigwig': '', 'pval_bigwig': '', 'overlap_hammock': json_item, 'idr_hammock': ''}
                                    else:
                                        dic[name]['overlap_hammock'] = json_item
                        else:
                            if name not in dic:
                                dic[name] = {'fc_bigwig': '', 'pval_bigwig': '', 'overlap_hammock': '', 'idr_hammock': ''}
                        if os.path.isdir(adpd + '/' + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + '/call-reproducibility_idr/execution/'):
                            for m in os.listdir(adpd + '/' + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + '/call-reproducibility_idr/execution/'):
                                if m.endswith('optimal_peak.narrowPeak.hammock.gz'):
                                    ftype = '\"hammock\"'
                                    tname = '\"'+j+'_idr_hammock\"'
                                    url = '\"http://mitra.stanford.edu/kundaje/projects/alzheimers_parkinsons/' + outputs + h + '/' + i + '/' + j + '/cromwell-executions/atac/' + k + '/call-reproducibility_idr/execution/' + m + '\"'
                                    mode = '\"full\"'
                                    json_item = '{\"type\": '+ftype+',\"url\": '+url+',\"mode\": '+mode+',\"name\": '+tname+'}'
                                    if name not in dic:
                                        dic[name] = {'fc_bigwig': '', 'pval_bigwig': '', 'overlap_hammock': '', 'idr_hammock': json_item}
                                    else:
                                        dic[name]['idr_hammock'] = json_item
                        else:
                            if name not in dic:
                                dic[name] = {'fc_bigwig': '', 'pval_bigwig': '', 'overlap_hammock': '', 'idr_hammock': ''}
    ...
    with open(filename,'w') as outfile:
        ...
        for item in dic:
            if item.strip('\"').split('_')[0] == group:
                for ftype in [track,'overlap_hammock','idr_hammock']:
                    if dic[item][ftype] != '':
                        if count != 0:
                            outfile.write(',')
                        outfile.write('\n'+dic[item][ftype])
