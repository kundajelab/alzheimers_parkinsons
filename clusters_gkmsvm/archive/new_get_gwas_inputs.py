    ...
    for index, row in snps.iterrows():
        ...
        for i,j in snps.iterrows():
            if j['ld_chr'] == chrom and j['ld_rsid'] != rsid and j['ld_rsid'] != '.' and j['ld_start'] >= start and j['ld_end'] <= end and len(j['minor']) == 1 and len(j['major']) == 1:
                ...
                if (j['ld_start'] - row['ld_start']) < 50 and (j['ld_start'] - row['ld_start']) >= 0:
                    print("Inserting "+j['ld_rsid']+" "+str(abs(j['ld_start'] - row['ld_start']))+" bases after "+rsid+" with minor allele "+j['minor'])
                elif (j['ld_start'] - row['ld_start']) > -50 and (j['ld_start'] - row['ld_start']) < 0:
                    print("Inserting "+j['ld_rsid']+" "+str(abs(j['ld_start'] - row['ld_start']))+" bases before "+rsid+" with minor allele "+j['minor'])
        assert len(major_seq) == 1000
        assert len(minor_seq) == 1000
        assert major_seq != minor_seq
