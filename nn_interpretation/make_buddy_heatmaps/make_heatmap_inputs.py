...
for cluster in range(1,25):
    for modeltype in ['classification','regression']:
        stats=pd.read_csv('../summaries_buddies/summaries.'+str(cluster)+".0."+modeltype+".txt",header=0,sep='\t')
...
for metric in metric_dict:
    outf=open("buddy.snps."+metric+".txt",'w')
...
outf=open('buddy.snps.metric.tally.txt','w')
outf.write('SNP\t'+'\t'.join([str(i) for i in range(1,25)])+'\n')
