...
def make_fasta(snps, cluster):
    effect_fasta = open('/home/groups/akundaje/soumyak/alzheimers_parkinsons/scoring_new_snps/explain_inputs/Cluster'+cluster+'.effect.fasta', 'w')
    noneffect_fasta = open('/home/groups/akundaje/soumyak/alzheimers_parkinsons/scoring_new_snps/explain_inputs/Cluster'+cluster+'.noneffect.fasta', 'w')
    counter = 0
    for index, row in snps.iterrows():
        ...
        if effect == row['ref']:
            ...
            noneffect_right = seq[500:]
            noneffect_left = seq[:(500-len(noneffect))]
        ...
        if effect == row['ref']:
            assert seq == effect_seq, '\n' + '\n'.join([seq[490:510], effect_seq[490:510], effect, noneffect, rsid])
        if noneffect == row['ref']:
            assert seq == noneffect_seq, '\n' + '\n'.join([seq[490:510], noneffect_seq[490:510], noneffect, effect, rsid])
